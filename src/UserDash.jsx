...
function UserDashboard() {
  ...
  const handleBuyFormSubmit = async (e) => {
    ...
    try {
      const email = localStorage.getItem('email');
      const response = await axios.post('http://localhost:5000/buy-book', {
        email,
  ...
  const handleAddToWishlist = async (bookId) => {
    try {
      const email = localStorage.getItem('email');
      const response = await axios.post('http://localhost:5000/wishlist', { email, bookId });
  ...
  const handleRemoveFromWishlist = async (bookId) => {
    try {
      const email = localStorage.getItem('email');
      const response = await axios.delete('http://localhost:5000/wishlist', { data: { email, bookId } });
