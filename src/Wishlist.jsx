...
function Wishlist({ email }) {
  ...
  const fetchWishlist = async () => {
    try {
      const response = await axios.get(`https://backend-bookstore1.onrender.com/wishlist?email=${email}`);
  ...
  const removeFromWishlist = async (bookId) => {
    try {
      await axios.post('https://backend-bookstore1.onrender.com/remove-from-wishlist', { email, bookId });
  ...
  const buyBook = async (bookId) => {
    try {
      await axios.post('https://backend-bookstore1.onrender.com/buy-book', { email, bookId });
